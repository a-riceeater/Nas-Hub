...
export class MetricCollector extends EventEmitter {
  current:Metric|null=null; private timer?:NodeJS.Timeout; private collecting=false; private lastPersist=0;private publicIPv4:string|null=null;private lastIPLookup=0;private diskDeviceCount=0;private lastDiskDiscovery=0; healthy=false;
  async start(){ const now=Date.now(); await db.insert(servers).values({id:LOCAL_SERVER_ID,name:hostname(),hostname:hostname(),operatingSystem:platform(),architecture:arch(),version:"0.1.0",createdAt:now,lastSeenAt:now}).onConflictDoUpdate({target:servers.id,set:{lastSeenAt:now}});await this.refreshDiskDeviceCount(now); await this.collect(); this.timer=setInterval(()=>void this.collect(),config.METRIC_COLLECTION_INTERVAL_MS); log.info("metric collector started"); }
  ...
  private async refreshDiskDeviceCount(timestamp:number){if(timestamp-this.lastDiskDiscovery<300000)return;this.lastDiskDiscovery=timestamp;try{this.diskDeviceCount=(await si.diskLayout()).length;}catch(error){log.warn({error},"disk device discovery unavailable");}}
  private async collect(){if(this.collecting)return;this.collecting=true;try{
    const [load,mem,fs,disks,fsStats,net,time,processes,temp]=await Promise.all([si.currentLoad(),si.mem(),si.fsSize(),si.disksIO().catch(()=>null),si.fsStats().catch(()=>null),si.networkStats(),si.time(),si.processes(),si.cpuTemperature().catch(()=>null)]);
    ...
    const loads=loadavg(), timestamp=Date.now();void this.refreshPublicIPv4(timestamp);void this.refreshDiskDeviceCount(timestamp);
    const systemUtilization=load.cpus.length?Math.min((loads[0]??0)/load.cpus.length*100,999):null;
    const diskDeviceCount=this.diskDeviceCount,diskBusyPercent=disks?.tWaitPercent==null?null:Math.min(100,Math.max(0,disks.tWaitPercent/Math.max(1,diskDeviceCount)));
    const m:Metric={timestamp,publicIPv4:this.publicIPv4,cpuPercent:load.currentLoad,systemUtilization,perCore:load.cpus.map(c=>c.load),load1:loads[0]??0,load5:loads[1]??0,load15:loads[2]??0,ramTotal:mem.total,ramUsed:mem.active,ramAvailable:mem.available,ramPercent:mem.total?mem.active/mem.total*100:0,swapTotal:mem.swaptotal,swapUsed:mem.swapused,diskTotal:disk.size,diskUsed:disk.used,diskAvailable:disk.available,diskPercent:disk.size?disk.used/disk.size*100:null,diskBusyPercent,diskDeviceCount,diskReadBps:fsStats?.rx_sec??null,diskWriteBps:fsStats?.wx_sec??null,networkRxBps:net.reduce((a,n)=>a+(n.rx_sec??0),0),networkTxBps:net.reduce((a,n)=>a+(n.tx_sec??0),0),uptime:time.uptime,processCount:processes.all,temperature:temp?.main??null,bootTime:timestamp-time.uptime*1000};
